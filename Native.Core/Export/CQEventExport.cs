...
namespace Native.App.Export
{
	...
	{	
		...
		{	
			return "9,online.smyhw.KQ.chat_backup";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("online.smyhw.KQ.chat_backup", 1, 9, "chat_backup", "1.0.0", 1, "smyhw", "指令:chat_backup <消息条数> 可以查看聊天记录，包括撤回信息(chat_backup可以缩写为cbp)。注意，指令使用双斜杠开头 ", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("online.smyhw.KQ.chat_backup", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("online.smyhw.KQ.chat_backup", AppData.CQLog);	
